...
bool Collator::create_shard_state() {
  ...
  ALWAYS_OR_UNREACHABLE(block::gen::t_ShardState.validate_ref(1000000, state_root), "Collator produced shard state passes structural validation", {{"shard", shard_.to_str()}});
  if (verify >= 2) {
  ...
  ALWAYS_OR_UNREACHABLE(r_state_update.is_ok(), "Collator Merkle update generation succeeds", {{"shard", shard_.to_str()}});
...
bool Collator::store_zero_state_ref(vm::CellBuilder& cb) {
  ...
  ALWAYS_OR_UNREACHABLE(root_hash == prev_blocks[0].root_hash, "Collator previous block root hash is consistent", {{"shard", shard_.to_str()}});
...
bool Collator::check_value_flow() {
  ALWAYS_OR_UNREACHABLE(value_flow_.validate(), "Collator value flow is balanced: in equals out", {{"shard", shard_.to_str()}});
...
bool Collator::create_block() {
  ...
  ALWAYS_OR_UNREACHABLE(block::gen::t_Block.validate_ref(10000000, new_block), "Collator produced block passes structural validation", {{"shard", shard_.to_str()}});
  if (verify >= 1) {
...
bool Collator::create_block_candidate() {
  ...
  if (need_out_msg_queue_broadcasts) {
    ...
    ALWAYS_OR_UNREACHABLE(new_state->get_hash() == state_root->get_hash(), "Collator Merkle update produces correct state hash", {{"shard", shard_.to_str()}});
  ...
  ALWAYS_OR_UNREACHABLE(block_candidate->data.size() <= consensus_config.max_block_size, "Collator block size within consensus limit", {{"shard", shard_.to_str()}, {"size", std::to_string(block_candidate->data.size())}});
