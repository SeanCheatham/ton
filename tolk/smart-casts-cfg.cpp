...
bool is_valid_lvalue_path(AnyExprV v, std::vector<SinkExpression>* out_sinks) {
  if (auto as_ref = v->try_as<ast_reference>()) {
    if (out_sinks) {
      if (LocalVarPtr var_ref = as_ref->sym->try_as<LocalVarPtr>()) {
  ...
  if (auto as_nn = v->try_as<ast_not_null_operator>()) {
    return is_valid_lvalue_path(as_nn->get_expr(), out_sinks);
  }
  if (auto as_dot = v->try_as<ast_dot_access>();
    ...
    bool inner_valid = is_valid_lvalue_path(as_dot->get_obj(), &inner_sinks);
    if (out_sinks) {
